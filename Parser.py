...
def write_params_to_json_file(id, length, osc_freqs, osc_bank_gains, noise_bank_gains, freq_curve_param, noise_curve_param):
    ...
    with open('Resources/DNA_Synth.json', 'r') as file:
        ...
        for i in range(len(noise_bank_gains)):
            data['pattrstorage']['slots'][str(id)]['data']['noiseband['+str(i+1)+']::A'] = [noise_curve_param[i][0]]
            data['pattrstorage']['slots'][str(id)]['data']['noiseband['+str(i+1)+']::B'] = [noise_curve_param[i][1]]
            data['pattrstorage']['slots'][str(id)]['data']['noiseband['+str(i+1)+']::duration'] = [length]
            data['pattrstorage']['slots'][str(id)]['data']['noiseband['+str(i+1)+']::static_gain'] = [noise_bank_gains[i]]
